...
- (BOOL)fileIsFirst{
    ...
    NSString * targetName = [[projectName componentsSeparatedByString:@"."] firstObject];
    
    
    NSString * addfileName = @"ZYSource.m";
    filePath = [NSString stringWithFormat:@"%@/%@/%@",projectDir,targetName,addfileName];
...
- (void)saveLocalPath{
    [[self class] getString:@"aaa"];
    
    BOOL isFirst = [self fileIsFirst];
    ...
    NSString * headeraddfileName = @"ZYSource.h";
    NSString* headerfilePath = [NSString stringWithFormat:@"%@/%@/%@",projectDir,targetName,headeraddfileName];
    
    NSString * contentaddfileName = @"ZYSource.m";
    NSString* contentfilePath = [NSString stringWithFormat:@"%@/%@/%@",projectDir,targetName,contentaddfileName];
    
    
    NSString * header = [self getHeaderText];
    NSString * content = [self getContentText];
    
    [header writeToFile:headerfilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
    [content writeToFile:contentfilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
    ...
    XCProject* project = [[XCProject alloc] initWithFilePath:filePath];
    NSError * error;
    NSString * fileData = [[NSString alloc] initWithContentsOfFile:contentfilePath encoding:NSUTF8StringEncoding error:&error];
    XCGroup* group = [project groupWithPathFromRoot:targetName];
    XCSourceFileDefinition* sourceFileDefinition = [[XCSourceFileDefinition alloc]
                                                    initWithName:contentaddfileName data:[fileData dataUsingEncoding:NSUTF8StringEncoding]
                                                    type:SourceCodeObjC];
    [group addSourceFile:sourceFileDefinition];
    XCSourceFile* sourceFile = [project fileWithName:contentaddfileName];
    XCTarget* examples = [project targetWithName:targetName];
    [examples addMember:sourceFile];
    [project save];
    
    
    fileData = [[NSString alloc] initWithContentsOfFile:headerfilePath encoding:NSUTF8StringEncoding error:&error];
    sourceFileDefinition = [[XCSourceFileDefinition alloc]
                            initWithName:headeraddfileName data:[fileData dataUsingEncoding:NSUTF8StringEncoding]
                            type:SourceCodeObjC];
    [group addSourceFile:sourceFileDefinition];
    sourceFile = [project fileWithName:headeraddfileName];
    examples = [project targetWithName:targetName];
    [examples addMember:sourceFile];
    [project save];
...
- (NSString *)getHeaderText{
    ...
    [string appendString:@"\n"];
    [string appendString:@"#define R [ZYSource getInstanse]\n"];
    [string appendString:@"@interface ZYSource : NSObject\n"];
    [string appendString:@"\n"];
    [string appendString:@"+ (ZYSource *)getInstanse;"];
    [string appendString:@"\n"];

    for(NSString * key in self.showArray){
        [string appendString:@"\n"];
        NSString * ocKey = [key stringByReplacingOccurrencesOfString:@" " withString:@"_"];
        [string appendFormat:@"/**\n *  @author kenny , %@\n *  \n *  Key:%@\n *  ",[NSDate date],key];
        
        for(StringModel * modle in self.stringArray){
            NSString * va = [modle.stringDictionary objectForKey:key];
            if(va){
                [string appendFormat:@"\n *  "];
                [string appendFormat:@"\n *  Value:%@;lan:%@",va,modle.identifier];
            }
        }
        [string appendFormat:@"\n */\n"];
        [string appendFormat:@"@property (nonatomic, strong, readonly) NSString * %@ ;",ocKey];
    }
...
- (NSString *)getContentText{
    NSMutableString * string = [NSMutableString new];
    [string appendString:@"#import \"ZYSource.h\"\n"];
    [string appendString:@"@implementation ZYSource\n\n"];

    [string appendString:@"\n"];

    [string appendString:@"#pragma mark - 初始化单例 \n\
...
+ (NSString *)getString:(NSString *)key{
    ...
    StringSetting *setting = [StringModel projectSettingByProjectPath:projectDir projectName:projectName];
    NSArray *lprojDirectorys = [StringModel lprojDirectoriesWithProjectSetting:setting project:projectDir];
    if(lprojDirectorys.count > 0){
        ...
        for (NSString *path in lprojDirectorys) {
            ...
            if (model) {
                ...
                if(value){
                    [st appendFormat:@"%@ : %@",model.identifier,value];
                }
            }
        }
    }
